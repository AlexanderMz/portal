<template>
  <v-container>
    ...
    <v-overlay
      ...
      :value="overlay"
    >
      <p>Generando dispersion de servicios</p>
      <v-progress-circular
        indeterminate
        size="64"
      ></v-progress-circular>
...
export default {
  ...
  methods: {
    ...mapActions("dispersion", ['getAllServices', 'generarTxtServices', 'limpiar']),
    cargarDatos3 () {
      this.loadTable = true
      this.getAllServices()
        .then(res => {
    ...
    generaUnoxUno () {
      this.overlay = true
      let data = {
        services: this.selectedToFile,
        g: this.isGenerate ? 1 : 0
      }
      this.generarTxtServices(data)
        .then(res => {
          if (res != null) {
  ...
  computed: {
    ...
    services () {
      return this.$store.state.dispersion.services
    },
    getValuesFromSet () {
